...
class Aes
{
    private mixed $key = "";
    private string $iv = "";
    private string $AesVersionCache = "aes_version_cache_hash";
    private string $AesVersionEnableCache = "aes_version_enable_hash";

    public function __construct($key = null, $iv = null)
    {
        if (is_null($key)) {
            $this->key = config('dev.EncryptKey');
        }
        if (is_null($iv)) {
     ...
     * @return bool
     */
    public function isEnableAes(RequestHeaders $requestHeaders): bool
    {
    ...
    /**
     * 根据{os+source+version}获取不同的aeskey
     * @param RequestHeaders $requestHeaders
     * @return mixed|string
     */
     ...
     * @throws Exception
     */
    public function aesEncrypt(string $plainText): string
    {
...
//            $str = $this->addPKCS7Padding($str);
            $iv = $this->iv;
            return openssl_encrypt($plainText, 'aes-128-cbc', $screct_key, 0, $iv);
        } catch (Exception $e) {
    ...
    public function aesDecrypt($cipherText)
    {
        ...
        try {
            if (strlen($key) == 0) {
                throw new Exception("AES解密失败，plainText=" . $cipherText . "，AES密钥为空。");
            }
            //AES, 128 模式加密数据 CBC
            $str = $cipherText;
            $screct_key = base64_decode($key);
            ...
            return openssl_decrypt($str, 'aes-128-cbc', $screct_key, 0, $iv);
        } catch (Exception $e) {
